...
ARIC_clinic <- transform(ARICevent, strkdate = pmin(C7_ED17ISC, C7_ED17HEM, na.rm = T)) %>% 
  ...
  dplyr::select(id, AGE = v1age01, SEX = v1_gender, SBP = v1_sbpa21, DBP = v1_sbpa22, DM = v1_diabts03,
                HDL = HDL_V1, TC = TOTCHOL_V1, BMI = v1_bmi01, SMK = v1_cigt01, LVEFz = lvef_cur, chfdiag, mi=hardchd, chf = C7_INCHF17,strkdate,strk,
                hardcvd, cvd_day_dif, dth = DEAD17, dthdate = C7_ENDDATED_BWH, v1date01,C7_DATE_INCHF17,C7_DATEMI) %>%
...
ARIC_dat1 <- ARIC %>% 
  dplyr::select(AGE = v1age, SEX = gender, SBP = v1sbp, DBP = v1dbp, HRX = v1htnmed, TG = v1trig, FG = GLUC_V1,
                id = id, SMK = v1cig, DM = v1diabetes, BMI = v1bmi, race = racegrp, TC = v1totchol, HDL = v1hdl) %>% 
  mutate(TP = 1)
ARIC_dat2 <- ARIC %>% 
  dplyr::select(AGE = v2age, SEX = gender, SBP = v2sbp, DBP = v2dbp, HRX = v2htnmed, TG = v2trig, FG = GLUC_V2,
                id = id, SMK = v2cig, DM = v2diabetes, BMI = v2bmi, race = racegrp, TC = v2totchol, HDL = v2hdl) %>% 
  mutate(TP = 2)
ARIC_dat3 <- ARIC %>% 
  dplyr::select(AGE = v3age, SEX = gender, SBP = v3sbp, DBP = v3dbp, HRX = v3htnmed, TG = v3trig, FG = GLUC_V3,
                id = id, SMK = v3cig, DM = v3diabetes, BMI = v3bmi, race = racegrp, TC = v3totchol, HDL = v3hdl) %>% 
  mutate(TP = 3)
ARIC_dat4 <- ARIC %>% 
  dplyr::select(AGE = v4age, SEX = gender, SBP = v4sbp, DBP = v4dbp, HRX = v4htnmed, TG = v4trig, FG = GLUC_V4,
                id = id, SMK = v4cig, DM = v4diabetes, BMI = v4bmi, race = racegrp, TC = v4totchol, HDL = v4hdl) %>% 
  mutate(TP = 4) 
