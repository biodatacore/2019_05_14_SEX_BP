...
MESAev <- transform(MESAev, cvd_day_dif = pmin(strktt, chftt, mitt, na.rm = T))%>%
  dplyr::select(id = idno, hardcvd, cvd_day_dif, dth, dthtime = dthtt, strktime = strktt, chftime = chftt, mitime = mitt, strk, chf, mi) %>%
  mutate_all(as.numeric)
