...
point <- cbind(point_f, point_m) %>%
  mutate(point_f = paste(sprintf("%.1f",p_f),"(",sprintf("%.1f",p_f - 1.96*se_f),",",sprintf("%.1f",p_f + 1.96*se_f),")", sep = ""),
         point_m = paste(sprintf("%.1f",p_m),"(",sprintf("%.1f",p_m - 1.96*se_m),",",sprintf("%.1f",p_m + 1.96*se_m),")", sep = ""),

         point_z = (p_f-p_m)/(se_f^2 + se_m^2)^0.5,
         dif = p_f-p_m,
         se = (se_f^2 + se_m^2)^0.5,
         difci = paste(sprintf("%.1f",dif),"(",sprintf("%.1f",dif - 1.96*se),",",sprintf("%.1f",dif + 1.96*se),")", sep = ""),
